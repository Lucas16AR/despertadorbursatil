...
## Decimoquinta tarea — RESUELTA (2026-07-22, ver PROGRESS.md para el detalle completo)

El pie del mensaje ahora tiene dos líneas separadas (`Datos:` / `Noticias:`), tomando las fuentes de noticias de `rss_news.FEEDS.keys()`. Con una salvedad respecto a lo especificado abajo: no se importa `rss_news` directamente desde `formatter.py` porque `tests.py` depende explícitamente de que `formatter.py` no tenga dependencias externas (`feedparser`, en este caso) para poder correr sin instalar nada — importar `rss_news` ahí tumbaba la suite. Se resolvió pasando la lista de fuentes como parámetro (`fuentes_noticias`) desde `main.py`, que sí ya depende de `rss_news`.
...
## Decimosexta tarea — RESUELTA (2026-07-22, ver PROGRESS.md para el detalle completo)

Aplicado el desfasaje recomendado en el punto 2 de abajo (los 4 cron en hora/cuarto exacto — `0 11`, `0 14`, `0 15`, `0 22` — ahora corren en `3 11`, `4 14`, `7 15`, `2 22`), sin tocar `15 20` ni `30 1` que ya estaban desfasados. El `case` del workflow que mapea `SCHEDULE` a `MOMENTO` se actualizó en el mismo commit — usa el string exacto del cron, así que un desfasaje sin actualizar el `case` haría caer esas 4 tandas al default (`pre_apertura`). **Pendiente real:** falta confirmar en el historial de GitHub Actions si el desfasaje efectivamente redujo la demora (punto 1 de abajo) — no se hizo en esta sesión, es la forma de saber si con esto alcanza o si hay que revisar la opción de cron externo más adelante.
...
## Decimotercera tarea — RESUELTA (2026-07-22, ver PROGRESS.md para el detalle completo)

`SYSTEM_PROMPT` de `macro_summary.py` reescrito para pedir bullets cortos. La primera corrida con datos reales dio 3 párrafos largos con un bullet pegado adelante — no era lo pedido — así que se ajustó el prompt para exigir explícitamente "UNA sola oración corta (máximo ~25 palabras)" por punto, y una segunda corrida contra Claude confirmó bullets realmente cortos sin perder contenido. De paso se sumó `html.escape(texto, quote=False)` al texto de Claude antes de insertarlo en el HTML del mensaje, para que un titular con `<`, `>` o `&` no rompa el parseo de Telegram (`quote=False` porque Telegram sólo exige escapar esos 3 caracteres — con el default de `html.escape` un apóstrofe como en "Moody's" salía como `Moody&#x27;s` en el mensaje).
...
## Decimocuarta tarea — RESUELTA (2026-07-22, ver PROGRESS.md para el detalle completo)

`inflacion.py` (nuevo, mismo patrón que `riesgo_pais.py`) trae inflación mensual e interanual y se sumó a "📊 Índices" y al prompt del resumen macro. **Bug encontrado en la fuente al validar con datos reales:** el endpoint documentado acá abajo (`.../inflacion-interanual`, con guiones) devuelve 404 — el endpoint real es `.../inflacionInteranual` (camelCase), confirmado con `curl` antes de commitear. UVA sigue sin sumarse (documentado más abajo como candidato a futura tarea, no confirmado por Capi todavía).
...
## Duodécima tarea — RESUELTA (2026-07-22, ver PROGRESS.md para el detalle completo)

Las 4 mejoras (A-D) implementadas y validadas con datos reales de dolarapi.com. `dolar.py` ahora trae las 7 casas (sumadas mayorista y tarjeta); `formatter.py` suma `_precio_texto()` (2 decimales cuando la casa los tiene, ej. MEP `$1506.40`), la leyenda de frescura de dolarapi.com después del bloque de cotizaciones, y `_miles()` para el separador de miles del MERVAL (`1.714.487`).
